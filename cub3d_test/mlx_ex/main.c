...
int		draw_grid(t_window *window)
{
	...
	while (i < window->row_cnt)
	{
		...
		{
			mlx_pixel_put(window->mlx, window->win, draw_position, i * (window->height / window->row_cnt), window->grid_color);
			draw_position++;
		}
		i++;
	}
	i = 1;
	while (i < window->column_cnt)
	{
		draw_position = 0;
		while (draw_position <= window->width)
		{
			mlx_pixel_put(window->mlx, window->win, i * (window->width / window->column_cnt), draw_position, window->grid_color);
			draw_position++;
		}
		i++;
	}
...
{
	mlx_pixel_put(window->mlx, window->win, window->player.x, window->player.y,
					window->player.color);
}
